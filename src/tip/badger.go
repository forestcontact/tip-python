...
func (bs *BadgerStorage) Find(prefix []byte) (keys [][]byte, values [][]byte, err error) {
	err = bs.db.Update(func(txn *badger.Txn) error {
		opts := badger.DefaultIteratorOptions
		opts.PrefetchValues = false
		opts.Prefix = prefix
...
func (bs *BadgerStorage) Filter(prefix []byte, fn func([]byte, []byte) bool) error {
	err := bs.db.Update(func(txn *badger.Txn) error {
		opts := badger.DefaultIteratorOptions
		opts.PrefetchValues = false
		opts.Prefix = prefix
...
func (bs *BadgerStorage) Set(key []byte, val []byte, ttl int64) error {
	return bs.db.Update(func(txn *badger.Txn) error {
		entry := badger.NewEntry(key, val)
		if ttl > 0 {
			entry = entry.WithTTL(time.Duration(ttl))
		}
		return txn.SetEntry(entry)
	})
}
...
func OpenBadgerEx(dir string) (*BadgerStorage, error) {
